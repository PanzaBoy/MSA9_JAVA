...
public class App {
	...
	public static Course<?> createCourse() {
		...
		case "직장인" -> {
			...
			yield newCouse.createWokerCourse(name, newCouse); 
		}
		case "학생" -> {
		...
		public static void addStudent() {
			...
			for (int i = 0; i < courseList.size(); i++) {
				...
				if (course.getName().equals(courseName)) {
					...
					//둥록할 과정은 직장인이라, Course<Woker> 로 되어 있고
		...
		public static void printStudent() {
			...
			for (int i = 0; i < courseList.size(); i++) {
				// 해당 과정의 수강생 목록
				List<Person> students = (List<Person>) courseList.get(i).getStudent();
				// 해당 과정의 과정명
				String courseName = courseList.get(i).getName();
				
				// 해당 과정의 수강생 목록을 확인하여, 입력한 이름과 일치하는 경우가 있으면
				// 해당 수강생이 수각하는 과정으로 인식하고 출력한다.
