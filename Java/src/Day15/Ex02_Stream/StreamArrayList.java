...
public class StreamArrayList {
	...
	public static void main(String[] args) {
		...
		personList.add( new Person("군영", 41) );
		personList.add( new Person("머슬영", 21) );
		personList.add( new Person("컹순영", 13) );
		personList.add( new Person("영이", 5) );
		personList.add( new Person("메슨영", 69) );
		personList.add( new Person("떨순영", 74) );
		...
		// - 메소드 호출시, 전달인자로 메소드 레퍼런스를 지정하여 넘겨줄 수 있다.
		// - 메소드를 통해 메소드를 전달하는 람다식을 대체하는 문법이다.
		// - 람다식을 간결하게 표현하기 위한 방법
		// (person) -> { return person.getNam(); }
		// person -> return person.getNam();
		// person::getNam();
		personList.stream()
